...
if __name__ == '__main__':
    ...
    parser.add_argument('--gate_init_bias', type=float, default=0.0,
                        help='Bias initialization for gated residual: 0.0→gate≈0.50, 1.0→gate≈0.73, 2.0→gate≈0.88')
    ...
    if str(args.model).lower() == 'itransformer_ps':
        ...
        print("-"*60)
        print(f"STAR Block:      {getattr(args, 'use_star', 0)}")
        print(f"CAC Block:       {getattr(args, 'use_cac', 0)}")
        gated = getattr(args, 'use_gated_residual', 0)
        if gated:
            print(f"Gated Residual:  enabled (Gateformer原文：bias=False, gate≈0.5)")
        else:
